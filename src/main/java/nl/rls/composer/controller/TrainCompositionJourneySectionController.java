...
public class TrainCompositionJourneySectionController {
	...
	public ResponseEntity<Resources<TrainCompositionJourneySectionDto>> getAll() {
		...
		Iterable<TrainCompositionJourneySection> trainCompositionJourneySectionList = trainCompositionJourneySectionRepository.findByOwnerId(ownerId);
		System.out.println("TrainCompositionJourneySectionController "+ownerId);
		...
		for (TrainCompositionJourneySection trainCompositionJourneySection : trainCompositionJourneySectionList) {
			trainCompositionJourneySectionDtoList.add(TrainCompositionJourneySectionDtoMapper.map(trainCompositionJourneySection));
		}
		Link trainCompositionJourneySectionsLink = linkTo(methodOn(TrainCompositionJourneySectionController.class).getAll()).withSelfRel();
		Resources<TrainCompositionJourneySectionDto> trainCompositionJourneySections = new Resources<TrainCompositionJourneySectionDto>(trainCompositionJourneySectionDtoList, trainCompositionJourneySectionsLink);
	...
	public ResponseEntity<TrainCompositionJourneySectionDto> getById(@PathVariable int id) {
		int ownerId = securityContext.getOwnerId();
		Optional<TrainCompositionJourneySection> optional = trainCompositionJourneySectionRepository.findByIdAndOwnerId(id, ownerId);
	...
	public ResponseEntity<TrainCompositionJourneySectionDto> create(@RequestBody TrainCompositionJourneySectionPostDto dto) {
		int ownerId = securityContext.getOwnerId();
		TrainCompositionJourneySection trainCompositionJourneySection = TrainCompositionJourneySectionDtoMapper.map(dto);
