...
public class TrainController {
	...
	public ResponseEntity<Resources<TrainDto>> getAll() {
		...
		Link trainsLink = linkTo(methodOn(TrainController.class).getAll())
				.withSelfRel();
		Resources<TrainDto> trains = new Resources<TrainDto>(
				trainDtoList, trainsLink);
	...
	public ResponseEntity<TrainDto> getById(@PathVariable int id) {
		...
		if (optional.isPresent()) {
			TrainDto trainDto = TrainDtoMapper
					.map(optional.get());
	...
	public ResponseEntity<TrainDto> create(@RequestBody TrainCreateDto dto) {
		...
		if (train != null) {
			...
			return ResponseEntity.created(
					linkTo(methodOn(TrainController.class).getById(train.getId()))
							.toUri())
					.body(resultDto);
		} else {
			return ResponseEntity.notFound().build();
	...
	public ResponseEntity<TrainDto> update(@PathVariable Integer id,
			@RequestBody TrainCreateDto trainCreateDto) {
		int ownerId = securityContext.getOwnerId();
		Optional<Train> optional = trainRepository.findByIdAndOwnerId(id, ownerId);
		if (optional.isPresent()) {
			...
			if (train != null) {
				...
				return ResponseEntity.created(linkTo(
						methodOn(TrainController.class).getById(train.getId()))
								.toUri())
						.body(dto);
			}
		}
	...
	public ResponseEntity<TrainDto> createSection(@PathVariable Integer id, 
		...
		train.getTrainCompositionJourneySections().add(trainCompositionJourneySection);
		train = trainRepository.save(train);
		...
		if (train != null) {
			...
			return ResponseEntity.created(
					linkTo(methodOn(TrainCompositionJourneySectionController.class).getById(trainCompositionJourneySection.getId()))
							.toUri())
					.body(resultDto);
		} else {
			return ResponseEntity.notFound().build();
